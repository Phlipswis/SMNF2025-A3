...
In @fig-plot_workload_kisystem ist der Vergleich des subjektiven Workloads zwischen Baseline und KI-Bedingung dargestellt. Für diesen Vergleich führten wir einen zweiseitigen t-Test mit abhängigen Stichproben durch. Ohne die Unterstützung von KI (*M* = `r round(mean(data_complete$b_dlr_wat_mean), 1)`, *SD* = `r round(sd(data_complete$b_dlr_wat_mean), 1)`) berichten die Teilnehmenden einen signifikant geringeren Workload als mit der Unterstützung von KI (*M* = `r round(mean(data_complete$ki_dlr_wat_mean), 1)`, *SD* = `r round(sd(data_complete$ki_dlr_wat_mean), 1)`, *t* = `r round(ttest_workload_with_without$statistic, 2)`, *p* \< .001). Dies deutet darauf hin, dass der Einsatz von KI die Teilnehmenden mental mehr beansprucht, als die Beurteilung der Posts ohne den Einsatz von KI.
...
In @fig-plot_hct_age ist der Zusammenhang zwischen dem Alter der Teilnehmenden und dem Vertrauen in das KI-System dargestellt. Für diesen Zusammenhang berechneten wir eine Pearson-Korrelation. Es besteht ein signifikanter Zusammenhang zwischen den beiden Variablen (*r*(`r cor_hct_alter$parameter`) = `r sub("^0", "", formatC(cor_hct_alter$estimate, format = "f", digits = 2))`, *p* = `r sub("^0", "", formatC(cor_hct_alter$p.value, format = "f", digits = 2))`). Das bedeutet, dass ein höheres Alter mit einem niedrigeren Vertrauen einherging und umgekehrt. Aufgrund des kleinen Korrelationskoeffizienten und der statistisch knappen Signifikanz ist der Zusammenhang jedoch als schwach einzuschätzen.
