...
An unserer Studie nahmen volljährige Personen (*N* = `r anzahl_teilnehmende`) teil. Das Alter der Teilnehmenden lag im Mittel bei `r round(alter_mittelwert, 1)` (*SD* = `r round(alter_sd, 1)`, *Mdn* = `r alter_median`) bei einer Altersspanne von `r alter_min` bis `r alter_max` Jahren. `r round(prozent_weiblich, 1)` % der Befragten gaben an, weiblich zu sein, `r round(prozent_maennlich, 1)` % männlich und `r round(prozent_andere, 1)` % ordneten sich einer andere Geschlechtskategorie zu. Zusätzlich wurde das Vorwissen der Teilnehmenden zu Künstlicher Intelligenz erfasst, wobei die Selbsteinschätzung auf einer Skala von 1 (sehr wenig) bis 5 (sehr viel) erfolgte. Der Mittelwert lag bei *M* = `r round(vorwissen_mittelwert, 1)` (*SD* = `r round(vorwissen_sd, 1)`), was auf ein durchschnittliches Kenntnisniveau innerhalb der Stichprobe hinweist. Im Rahmen des Between-Subjects-Designs untersuchten wir zwei unabhängige Gruppen. Eine Gruppe (*n* = `r anzahl_empfehlend` erhielt eine empfehlende KI, während die andere Gruppe (*n* = `r anzahl_evaluativ`) mit einer evaluativen KI interagierten. Diese Einteilung ermöglichte den Vergleich der unterschiedlichen KI-Systeme hinsichtlich einer weiteren Variable. Für die Korrelationsanalyse sowie die Tests im Rahmen des Within-Subjects-Designs wurde die gesamte Stichprobe als eine einheitliche Gruppe betrachtet.
...
Unsere Deskriptive Statistik ist in @tbl-deskriptive_statistik dargestellt. Nach den Kriterien von George und Mallery [-@george-2010] liegt die Skala zum subjektiven Workload sowohl mit als auch ohne KI im Bereich einer akzeptablen Konsistenz (Cronbachs Alpha = `r sub("^0", "", as.character(alpha_b_dlr_wat))`). Unsere Skalen des User Experience Questionaire, sowohl für die evaluative als auch für die empfehlende KI liegen mit einem Cronbachs Alpha über .9 im Bereich der exzellenten Konsistenz. Gleiches gilt für den Human-Computer Trust Index. Damit verfügen alle verwendeten Messinstrumente über eine ausreichende Reliabilität, um in der weiteren Analyse aussagekräftige Ergebnisse zu liefern. Der Mittelwert des Workloads mit KI (*M* = `r round(mean(data_complete$ki_dlr_wat_sum), 1)`) liegt über dem des Workloads ohne KI (*M* =`r round(mean(data_complete$b_dlr_wat_sum), 1)`), genau wie die User Experience mit empfehlender KI (*M* =`r round(mean(data_UXinverted$ueq_r_mean, na.rm = TRUE), 1)`) über der mit evaluativer KI (*M* =`r round(mean(data_UXinverted$ueq_e_mean, na.rm = TRUE), 1)`) liegt. Das Vertrauen in das System, gemessen mithilfe der HCT-Skala, liegt im Mittel bei *M* = `r round(mean(data_complete$hct_mean), 1)` auf einer Skala von 1 bis 6. Dies lässt ein mittleres bis leicht positives Vertrauen der gesamten Stichprobe vermuten. Allerdings zeigt die Standardabweichung (*SD* = `r round(sd(data_complete$hct_mean), 1)`), dass die Bewertung des Vertrauens deutlich streut.
...
```
**T-Test abhängig**
T: `r round(ttest_workload_with_without$statistic, 2)` Df: `r round(ttest_workload_with_without$parameter, 0)` p:`r ttest_workload_with_without$p.value`
Der gepaarte t-Test vergleicht die Mittelwerte der Summe DLR WAT zwischen zwei Bedingungen — Baseline und KI — bei denselben Teilnehmern.

T = -4.34 bedeutet, dass die Werte in der KI-Bedingung im Durchschnitt deutlich höher sind als in der Baseline (da die Differenz Baseline minus KI negativ ist).

Df = 175 entspricht der Anzahl der Paare minus 1.

p ≈ 2.37 × 10⁻⁵ ist sehr klein, was auf eine hochsignifikante Differenz hinweist.


**T-Test unabhängig**
T: `r round(ttest_ueq_system$statistic, 2)` Df: `r round(ttest_ueq_system$parameter, 0)` p:`r ttest_ueq_system$p.value`
Da der p-Wert kleiner als das übliche Signifikanzniveau von 0.05 (oder sogar 0.01) ist, kannst du die Nullhypothese ablehnen.
Das heißt: Das System "E" hat eine signifikant kleinere durchschnittliche UEQ-Summe als System "R" (bei der Annahme, dass deine Hypothese alternative = "less" ist).

**Korrelation**
Korrelationskoeffizient r: `r round(cor_hct_alter$estimate, 2)`  
p-Wert: `r signif(cor_hct_alter$p.value, 3)`  
Konfidenzintervall: (`r round(cor_hct_alter$conf.int[1], 2)`, `r round(cor_hct_alter$conf.int[2], 2)`)
Es gibt einen statistisch signifikanten, schwachen negativen Zusammenhang zwischen Alter und HCT. Der Effekt ist klein, und das Konfidenzintervall zeigt, dass die Unsicherheit relativ groß ist — die Korrelation könnte also auch nahe null sein. Dennoch deutet das Ergebnis darauf hin, dass mit steigendem Alter der HCT tendenziell etwas niedriger sein könnte.

```{r my-plot, mittelwertvergleich_workload_with_without_KI}
#| echo: false
...
ggplot(data_long, aes(x = system, y = value)) +
  ...
  labs(x = "System", y = "Summe DLR WAT", title = "Mittelwertvergleich: Baseline vs. KI") +
  theme_minimal()
```


```{r my-plot, mittelwertvergleich_ueq_system}
#| echo: false
ggplot(data_KIsystem, aes(x = system , y = ueq_mean)) +
  stat_summary(fun = mean, geom = "point", size = 3, color = "blue") +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.2) +
  labs(x = "KI-System", y = "UEQ-Mittelwert", title = "Mittelwertvergleich: UEQ nach KI-System") +
  theme_minimal()
```

```{r my-plot, fig.cap="Scatterplots von Age vs. HCT-Variablen mit Regressionslinien", fig.height=4, fig.width=6, warning=FALSE, message=FALSE}
#| echo: false
ggplot(data_complete, aes(x = age, y = hct_mean)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", color = "blue", se = FALSE) +
  labs(title = "Alter vs. hct_mean", x = "Alter", y = "hct_mean") +
  theme_minimal()
```
