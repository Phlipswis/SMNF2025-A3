...
```{r tbl-ksehfbk, results='asis' echo: false}
...
df <- data.frame(
  ...
  CA = c("Cronbachs Alpha",
                 " ",
                 " ",
                 alpha_ueq,
                 alpha_hct_r),
  mittelwert = c("Mittelwert",
                 mean(data_filtered$b_dlr_wat_w),
                 mean(data_filtered$ki_dlr_wat_w),
                 mean(data_filtered$ueq_sum),
                 mean(data_filtered$hct_r_sum)),
  median = c("Median",
  ...
  sd = c("Standartabweichung",
                 sd(data_filtered$b_dlr_wat_w),
                 sd(data_filtered$ki_dlr_wat_w),
                 sd(data_filtered$ueq_sum),
                 sd(data_filtered$hct_r_sum)),
  Prozent = c("Anzahl oder Prozentzahlen für die verschiedenen Kategorien",
                 paste(round(prop_b, 1), collapse = "; "),
                 paste(round(prop_ki, 1), collapse = "; "),
                 paste(round(prop_ueq, 1), collapse = "; "),
                 paste(round(prop_hct, 1), collapse = "; "))
...
data_filtered <- data_filtered %>%
  mutate(age_group = ifelse(age <= 30, "jünger", "älter"))

describeBy(data_filtered$ueq_sum, group = data_filtered$age_group)

t.test(data_filtered$ueq_sum ~ data_filtered$age_group, alternative = "less")

library(ggplot2)
ggplot(data_filtered, aes(x = age_group, y = ueq_sum)) +
  geom_point()
  theme_minimal() +
  labs(x = "Altersgruppe", y = "UEQ-Summe", title = "Mittelwertvergleich: UEQ nach Alter")
